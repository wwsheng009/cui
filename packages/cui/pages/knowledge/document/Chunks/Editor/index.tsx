...
const ChunkEditor: React.FC<ChunkEditorProps> = ({ chunkData, onSave }) => {
	...
	return (
		<div className={styles.chunkEditor}>
			{/* 标题区域 */}
			<div className={styles.header}>
				<div className={styles.titleSection}>
					<Icon name='material-edit_note' size={16} />
					<h3 className={styles.title}>{is_cn ? 'Chunk 内容编辑' : 'Chunk Content Editor'}</h3>
				</div>
			...
			<div className={styles.textSection}>
				...
				{isEditing ? (
					...
					<div className={styles.textDisplay}>
						<Text style={{ whiteSpace: 'pre-wrap', lineHeight: 1.6 }}>{chunkData.text}</Text>
					</div>
				)}
			</div>
			...
			<div className={styles.voteSection}>
				...
				<div className={styles.voteStats}>
					<div className={styles.voteItem}>
						<Icon name='material-thumb_up' size={14} className={styles.upvoteIcon} />
						<Text>
							{is_cn ? '赞同' : 'Upvotes'}: {chunkData.upvotes}
						</Text>
					</div>
					<div className={styles.voteItem}>
						<Icon name='material-thumb_down' size={14} className={styles.downvoteIcon} />
						<Text>
